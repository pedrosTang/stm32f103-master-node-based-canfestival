...
UNS8 proceedSDO (CO_Data* d, Message *m)
{
	...
	if(offset) while (offset <= lastIndex) {
		if (d->objdict[offset].bSubCount <= 1) {
			...
			printf("%s/n","Subindex 1  not found at index");
			return 0xFF;
		}
