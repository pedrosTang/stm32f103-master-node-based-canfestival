...
//  
//}
	TIMER_HANDLE PDO_Delay_Timer;
UNS8 proceedSYNC(CO_Data* d)
{
	UNS32 pdelay_index = 0x2000; // 对象字典索引
	UNS8 pdelay_subindex = 0x00; // 对象字典子索引

	TIMEVAL delayTime=0;
	UNS8 dataType;
	UNS32 pExpectedSize = 0x4;
	UNS8 errorCode;
  ...
  if(! d->CurrentCommunicationState.csPDO) 
	...
	getODentry(d, pdelay_index, pdelay_subindex, &delayTime, &pExpectedSize, &dataType, errorCode);
	 //printf("延迟时间为%lu",delayTime);
	if (errorCode != OD_SUCCESSFUL || delayTime == 0) {
        // 如果获取延迟时间失败或延迟时间为0，则立即发送PDO
        res = _sendPDOevent(d, 1 /*isSyncEvent*/ );
        (*d->post_TPDO)(d);
...
PDO_Delay_Timer = SetAlarm(d, 0, &sendPDOCallback, delayTime, 0);
  return res;
}
void sendPDOCallback(CO_Data* d, UNS32 timerId)
{
    // 调用 _sendPDOevent 并传递参数
    _sendPDOevent(d, 1);
